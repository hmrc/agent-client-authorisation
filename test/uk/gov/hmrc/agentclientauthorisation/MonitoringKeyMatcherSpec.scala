...
class MonitoringKeyMatcherSpec extends UnitSpec {
  ...
  "MonitoringKeyMatcher" should {
    ...
    "match URI to known pattern and produce key with placeholders replaced" in {
      ...
      tested.findMatchingKey(
        "http://foo.protected.mdtp/foo/PERSONAL-INCOME-RECORD/agent/ARN123456/client/GHZ8983HJ") shouldBe Some(
        "foo-PIR")
      tested.findMatchingKey(
        "http://foo.protected.mdtp/foo/afi/agent/ARN123456/client/GHZ8983HJ") shouldBe Some(
        "foo-AFI")
      tested.findMatchingKey(
        "http://foo.protected.mdtp/foo/service/PERSONAL-INCOME-RECORD/clientId/GHZ8983HJ") shouldBe Some(
