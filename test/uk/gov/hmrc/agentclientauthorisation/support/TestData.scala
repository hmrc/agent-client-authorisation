...
trait TestData {
  ...
  val allInvitations = List(
    Invitation(mtdSaPendingInvitationDbId, mtdSaPendingInvitationId, arn, Service("HMRC-MTD-IT"), mtdItId1.value, "postcode", nino1.value, "ni", events = List(StatusChangeEvent(now(), Pending))),
    Invitation(mtdSaAcceptedInvitationDbId, mtdSaAcceptedInvitationId, arn, Service("HMRC-MTD-IT"), mtdItId1.value, "postcode", nino1.value, "ni", events = List(StatusChangeEvent(now(), Accepted))),
    Invitation(otherRegimePendingInvitationDbId, otherRegimePendingInvitationId, arn, Service("mtd-other"), mtdItId1.value, "postcode", nino1.value, "ni", events = List(StatusChangeEvent(now(), Pending)))
  )
