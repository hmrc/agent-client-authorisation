...
class RoutineJobSchedulerISpec
    extends TestKit(ActorSystem("testSystem")) with UnitSpec with MongoAppAndStubs with EmailStub with IntegrationPatience {
  ...
  "RoutineJobScheduler" should {
    ...
    "remove the details for email for each invitation where the period for retaining these has expired" in {
      ...
      val actorRef =
        system.actorOf(Props(new RoutineScheduledJobActor(schedulerRepository, invitationsService, repeatInterval = 60, altItsaExpiryEnable = true)))
    ...
    "find invitations that are about to expire and send one email per agent" in {
      ...
      await(
        Future.sequence(
          List(aboutToExpireInvitation1, aboutToExpireInvitation2, aboutToExpireInvitation3)
    ...
    "cancel partialAuth invitations after the partial-auth expiry period has elapsed and send an audit event for each one" in {
      ...
      await(
        Future.sequence(
          List(partialAuthInvitation1, partialAuthInvitation2, partialAuthInvitation3)
            .map(invitationsRepository.collection.insertOne(_).toFuture())
        )
      )
      ...
      eventually {
        ...
        verifyAuditRequestSent(
          ...
          Map("transactionName" -> "hmrc expired agent:service authorisation", "path" -> ""),
          Map(
