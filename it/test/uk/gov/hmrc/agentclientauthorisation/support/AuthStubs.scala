...
trait AgentAuthStubs extends BasicUserAuthStubs {
  ...
  def authorisedAsValidClientWithAffinityGroup[A](request: FakeRequest[A], clientVatEnrolKey: String, clientITEnrolKey: String)(implicit
    hc: HeaderCarrier
  ): FakeRequest[A] =
    authenticatedClientAffinityGroup(request, clientVatEnrolKey, clientITEnrolKey)

  def authorisedAsNinoClient[A](request: FakeRequest[A], nino: Nino, affinityGroup: String = "Individual")(implicit
    hc: HeaderCarrier
  ): FakeRequest[A] = {
    givenAuthorisedFor(
      payload = """
  ...
  def authenticatedClientAffinityGroup[A](request: FakeRequest[A], vatEnrolKey: String, itEnrolKey: String, confidenceLevel: Int = 200)(implicit
    hc: HeaderCarrier
  ): FakeRequest[A] = {
    givenAuthorisedFor(
      payload = """
  ...
  def authenticatedClient[A](
    ...
    confidenceLevel: Int = 250,
    affinityGroup: String = "Individual",
    enrolments: Set[AuthEnrolment]
