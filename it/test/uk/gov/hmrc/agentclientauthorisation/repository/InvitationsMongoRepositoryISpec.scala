...
class InvitationsMongoRepositoryISpec
  ...
  "update" should {
    "create a new StatusChangedEvent" in {
      ...
      inside(updated) { case Invitation(created._id, _, _, _, _, _, _, _, _, _, _, _, _, events) =>
  ...
  "setRelationshipEnded" should {
    "set isRelationshipEnded flag to be true" in {
      ...
      inside(ended) { case Invitation(created._id, _, _, _, _, _, _, _, _, isRelationshipEnded, _, _, _, _) =>
  ...
  "listing by arn" should {
    ...
    "return previously created and updated elements" in {
      ...
      inside(list(1)) { case Invitation(_, _, Arn(`arn`), _, Service.MtdIt, _, _, _, _, _, _, _, _, List(StatusChangeEvent(date1, Pending))) =>
        date1 shouldBe now
      }
    }
    ...
    "not return elements created before the specified date" in {
      ...
      val invitationITSA1 = invitationITSA.copy(suppliedClientId = nino, clientId = clientId1)
      val invitationITSA2 = invitationITSA.copy(suppliedClientId = nino1, clientId = clientId2)
      val invitationITSA3 = invitationITSA.copy(suppliedClientId = nino2, clientId = clientId1)
  ...
  "listing by clientId" should {
    ...
    "return a multiple requests for the same client" in {
      ...
      inside(requests(1)) { case Invitation(_, _, `secondAgent`, _, `service`, _, _, _, _, _, _, _, _, List(StatusChangeEvent(dateValue, Pending))) =>
        dateValue shouldBe now
      }
    }
  ...
  private def addInvitations(startDate: LocalDateTime, invitations: Invitation*) =
    await(Future sequence invitations.map { case Invitation(_, _, arnValue, clientType, service, clientId, suppliedClientId, _, _, _, _, _, _, _) =>
