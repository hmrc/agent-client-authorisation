...
class ClientRejectInvitationsControllerISpec extends BaseISpec {
  ...
  "PUT /clients/UTR/:utr/invitations/received/:invitationId/reject" should {
    ...
    "return 204" when {
      "successfully rejecting an invitation" in new StubsSuccessfulScenariosTest(business, Service.Trust) {
    ...
    "return 404" when {
      "attempting to reject an invitation that does not exist" in {
    ...
    "return 403" when {
      "attempting to reject an invitation that is not for the current logged in client" in {
        givenClientAll(mtdItId, vrn, nino, utr)

        val result: Future[Result] = trustController.rejectInvitation(Utr("123456789"), InvitationId("D123456789"))(request)
