...
class ClientFiltersByStatusISpec extends FeatureSpec with GivenWhenThen with Matchers with MongoAppAndStubs with Inspectors with Inside with Eventually {
  ...
  private def agencySendsSeveralInvitations(agency: AgencyApi, regime: String): Unit = {
    ...
    Then(s"the Agency should see 2 pending invitations to Client $mtdClientId")
