...
class ClientFiltersByStatusApiPlatformISpec
  ...
  feature("Clients can filter") {
    ...
    scenario(s"on the status of invitations - ${Service.MtdIt.id}") {
      ...
      given().client(clientId = nino, canonicalClientId = mtdItId1).getTradingName(nino, "Trader")
    ...
    scenario(s"on the status of invitations - ${Service.PersonalIncomeRecord.id}") {
      ...
      given()
        ...
        .hasABusinessPartnerRecord(nino1)
      anAfiRelationshipIsCreatedWith(arn, nino1)
      given().client(clientId = nino1).getAgencyEmailViaClient(arn)
      given().client(clientId = nino1).givenGetAgentNameViaClient(arn)
      given().client(clientId = nino1).givenMtdItIdToNinoForClient(mtdItId1, nino1)
      given().client(clientId = nino1).givenCitizenDetails(nino1, "10102010")
      given().client(clientId = nino1, canonicalClientId = mtdItId1).getTradingName(nino1, "Trader")
      given().agentAdmin(arn).givenAuthorisedAsAgent(arn).givenGetAgentName(arn)
