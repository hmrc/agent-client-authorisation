...
trait DesStubs[A] {
  ...
  def hasVatCustomerDetails(isEffectiveRegistrationDatePresent: Boolean): A = {
    stubFor(get(urlEqualTo(s"/vat/customer/vrn/${encodePathSegment(clientId.value)}/information"))
      .withHeader("authorization", equalTo("Bearer secret"))
      .withHeader("environment", equalTo("test"))
      .willReturn(aResponse()
        .withStatus(200)
        .withBody(s"""{
                     ...
                     |         ${if (isEffectiveRegistrationDatePresent) ""","effectiveRegistrationDate" : "2017-04-01"""" else ""}
                     |      },
                     |      "bankDetails" : {
                     |         "sortCode" : "16****",
                     |         "accountHolderName" : "***********************",
