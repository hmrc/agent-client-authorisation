...
//    var links = HalLinks(Vector(HalLink("self", routes.ClientInvitationsController.getInvitation(invitation.clientId.value, invitation.id.stringify).url)))
//
//    agencyLink(invitation).foreach(href => links = links ++ HalLink("agency", href))
//
//    if (invitation.status == Pending) {
//      links = links ++ HalLink("accept", routes.ClientInvitationsController.acceptInvitation(invitation.clientId.value, invitation.id.stringify).url)
//      links = links ++ HalLink("reject", routes.ClientInvitationsController.rejectInvitation(invitation.clientId.value, invitation.id.stringify).url)
//    }
//
...
//    invitations.map { i => HalLink("invitations", routes.ClientInvitationsController.getInvitation(i.clientId.value, i.id.stringify).toString)}.toVector
//  }
//
...
//
//    val links = Vector(HalLink("self", routes.ClientInvitationsController.getInvitations(clientId.value, status).url)) ++ invitationLinks(invitations)
