...
trait ClientInvitationsHal {
  ...
  def toHalResource(invitation: Invitation): HalResource = {
    var links = HalLinks(Vector(HalLink("self", routes.ClientInvitationsController.getInvitation(invitation.clientId, invitation.id.stringify).url)))

    agencyLink(invitation).foreach(href => links = links ++ HalLink("agency", href))

    if (invitation.status == Pending) {
      links = links ++ HalLink("accept", routes.ClientInvitationsController.acceptInvitation(invitation.clientId, invitation.id.stringify).url)
      links = links ++ HalLink("reject", routes.ClientInvitationsController.rejectInvitation(invitation.clientId, invitation.id.stringify).url)
    }
  ...
  def toHalResource(invitations: Seq[Invitation], clientId: String, status: Option[InvitationStatus]): HalResource = {
    ...
    val links = Vector(HalLink("self", routes.ClientInvitationsController.getInvitations(clientId, status).url)) ++ invitationLinks(invitations)
