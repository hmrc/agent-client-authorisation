...
abstract class BaseClientInvitationsController[T <: TaxIdentifier](
  ...
  protected def getDetailsForClient(clientId: ClientIdentifier[T],
  ...
  protected def acceptInvitation(clientId: ClientIdentifier[T],
        ...
        invitation =>
          invitationsService.acceptInvitation(invitation).andThen {
            case Success(Right(x)) =>
              auditService.sendAgentClientRelationshipCreated(
                invitationId.value,
                x.arn,
                clientId,
                invitation.service)
  ...
  protected def getInvitation(clientId: ClientIdentifier[T],
                              invitationId: InvitationId)(
      implicit ec: ExecutionContext,
      hc: HeaderCarrier,
      request: Request[Any],
  ...
  protected def getInvitations(taxId: ClientIdentifier[T],
                               status: Option[InvitationStatus])(
      implicit ec: ExecutionContext,
      authTaxId: ClientIdentifier[T]) = forThisClient(taxId) {
    invitationsService.clientsReceived(supportedService, taxId, status) map (
        results => Ok(toHalResource(results, taxId, status)))
  }
  ...
  protected def actionInvitation(
    ...
    invitationsService.findInvitation(invitationId) flatMap {
      case Some(invitation)
          if matchClientIdentifiers(invitation.clientId, clientId) =>
  ...
  protected def forThisClient(taxId: ClientIdentifier[T])(
      block: => Future[Result])(implicit ec: ExecutionContext,
                                authTaxId: ClientIdentifier[T]) =
    if (authTaxId.value != taxId.value) Future successful NoPermissionOnClient
    else block

  private def matchClientIdentifiers(
