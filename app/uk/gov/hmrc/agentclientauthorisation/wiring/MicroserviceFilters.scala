...
class MicroserviceFilters @Inject()(
    ...
    extends DefaultHttpFilters(
      Seq(metricsFilter,
          ...
          auditFilter,
          loggingFilter,
          cacheFilter) ++ MicroserviceFilters
...
class WhitelistFilter @Inject()(configuration: Configuration)
  ...
  override val whitelist: Seq[String] = whitelistConfig(
    "microservice.whitelist.ips")
  ...
  private def whitelistConfig(key: String): Seq[String] =
    new String(
      Base64.getDecoder().decode(configuration.getString(key).getOrElse("")),
      "UTF-8").split(",")
}
