...
case class AgentRequest[A](arn: Arn, request: Request[A])
...
class AuthActions @Inject()(metrics: Metrics, val authConnector: AuthConnector)
  ...
  private val affinityGroupAllEnrolls: Retrieval[
    ~[Option[AffinityGroup], core.Enrolments]] = affinityGroup and allEnrolments
    ...
    Action.async { implicit request ⇒
      authorised(AuthProvider).retrieve(affinityGroupAllEnrolls) {
        case Some(affinityG) ~ allEnrols ⇒
          (isAgent(affinityG),
           extractEnrolmentData(allEnrols.enrolments, agentEnrol, agentEnrolId)) match {
  ...
  def onlyForClients[T <: TaxIdentifier](service: Service,
    ...
    authorised(AuthProvider).retrieve(allEnrolments) { allEnrols =>
      val clientId = extractEnrolmentData(allEnrols.enrolments,
                                          service.enrolmentKey,
                                          clientIdType.enrolmentId)
      if (clientId.isDefined)
        action(request)(
          ClientIdentifier(clientIdType.createUnderlying(clientId.get)))
